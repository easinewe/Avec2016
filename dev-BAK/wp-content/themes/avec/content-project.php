...
?>
		...
		<div id="project-content-container" class="content-container">
			<div id="project-hero" class="hero" >
				<div id="title-box" style="background-color:<?php echo ( get_post_meta( get_the_ID(), 'color_space', true) )? '#' . ltrim(get_post_meta( get_the_ID(), 'color_space', true), '#'): '#000000'; ?>;background-image:url('<?php echo get_template_directory_uri(); ?>/images/loader.gif'); background-repeat:no-repeat; background-position:90% 90%;">
					<h1><?php the_title(); ?></h1>
				...
				<?php endif; ?>
				<img id="hero_image" class="big-unshifted" src="<?php $image_source = wp_get_attachment_image_src( get_post_thumbnail_id( ), 'xsmall' ); echo $image_source[0]; ?>" <?php if ($hero_image) : ?>class="hot" onclick="callCarousel(<?php $GLOBALS['carousel_count']++; echo $GLOBALS['carousel_count']; ?>);"<?php endif; ?>  />
				<script type='text/javascript'> window.onload=function(){ jQuery('#image_load_holder').append('<img id="pre_hero_image" src="<?php $image_source = wp_get_attachment_image_src( get_post_thumbnail_id( ), 'xlarge' ); echo $image_source[0]; ?>" onload="heroShift();" >'); }; </script>
			</div>
			...
			<!--// GALLERY //-->
			<div id="project-masonry-wrapper" class="masonry-wrapper">
				<?php echo DEVONA_rewriteGallery( get_post_meta( get_the_ID(), 'project_gallery', true)  );?>
			</div>
			<br class="clearfix"/>
		</div>
